...
from evaluate import get_colors, get_high_res_texture, get_mapping_area
...
def apply_edit(model_F_atlas,
    ...
    edited_tex1, texture_orig1 = get_high_res_texture(1000, 0, 1, 0, 1,
    ...
    imageio.imwrite(
        ...
        (np.concatenate(
            (texture_edit_im1, alpha_im1[:, :, np.newaxis]), axis=2) *
         255).astype(np.uint8))
    edited_tex1 = torch.from_numpy(1 - alpha_im1).unsqueeze(
        -1) * texture_orig1 + torch.from_numpy(alpha_im1).unsqueeze(
            -1) * texture_edit_im1

    edited_tex1_only_edit = torch.cat(
        (edited_tex1_only_edit, torch.from_numpy(alpha_im1).unsqueeze(-1)),
        dim=-1)
    edited_tex2_only_edit = torch.from_numpy(texture_edit_im2)
    imageio.imwrite(
        "%s/texture_edit_user2.png" % output_folder_final,
        (np.concatenate(
            (texture_edit_im2, alpha_im2[:, :, np.newaxis]), axis=2) *
         255).astype(np.uint8))
    edited_tex2 = torch.from_numpy(1 - alpha_im2).unsqueeze(
        -1) * texture_orig2 + torch.from_numpy(alpha_im2).unsqueeze(
            -1) * texture_edit_im2

    edited_tex2_only_edit = torch.cat(
        (edited_tex2_only_edit, torch.from_numpy(alpha_im2).unsqueeze(-1)),
        dim=-1)
            ...
            for i in range(len(relisa)):
                ...
                try:
                    masks1[np.ceil(pointsy1).astype((np.int64)),
                           np.ceil(pointsx1).astype((np.int64))] = 1
                    masks1[np.floor(pointsy1).astype((np.int64)),
                           np.floor(pointsx1).astype((np.int64))] = 1
                    masks1[np.floor(pointsy1).astype((np.int64)),
                           np.ceil(pointsx1).astype((np.int64))] = 1
                    masks1[np.ceil(pointsy1).astype((np.int64)),
                           np.floor(pointsx1).astype((np.int64))] = 1
                ...
                only_mapped_texture[relisa[i][relevant1_only_edit],
                                    reljsa[i][relevant1_only_edit], :,
                                    f] = rgb_only_edit

                only_mapped_texture2[relisa[i][relevant2_only_edit],
                                     reljsa[i][relevant2_only_edit], :,
                                     f] = rgb_only_edit2
            # see details in Section 3.4 in the paper
            foreground_edit_cur = only_mapped_texture[:, :, :3,
                                                      f]  # denoted in the paper by c_{ef}
            foreground_edit_cur_alpha = only_mapped_texture[:, :, 3,
                                                            f][:, :, np.
                                                               newaxis]  # denoted by \alpha_{ef}

            background_edit_cur = only_mapped_texture2[:, :, :3,
                                                       f]  # denoted in the paper by c_{eb}
            background_edit_cur_alpha = only_mapped_texture2[:, :, 3,
                                                             f][:, :, np.
                                                                newaxis]  # denoted in the paper by \alpha_{eb}

            video_frame_cur = video_frames[:, :, :, f].cpu().clone().numpy(
            )  # denoted in the paper by \bar{c}_{b}

            # Equation (15):
            video_frame_cur_edited1 = foreground_edit_cur * (
                foreground_edit_cur_alpha) + video_frame_cur * (
                    1 - foreground_edit_cur_alpha)  #\bar{c}_b
            video_frame_cur_edited2 = background_edit_cur * (
                background_edit_cur_alpha) + video_frame_cur * (
    ...
    writer_edit = imageio.get_writer("%s/edited_%s.mp4" %
                                     (output_folder_final, vid_name),
                                     fps=10)
    ...
    for i in range(number_of_frames):
        ...
        writer_edit.append_data(
            (rgb_edit_video[:, :, :, i] * (255)).astype(np.uint8))

    writer_edit.close()


def texture_edit_from_frame_edit(edit_frame, f, model_F_mapping1,
                                 model_F_mapping2, model_alpha,
                                 number_of_frames, mask_frames,
                                 edit_frame_foreground, edit_frame_background,
                                 device):
    ...
    relis_i, reljs_i = torch.where(
        torch.ones(edit_frame.shape[0], edit_frame.shape[1]) > 0)
    ...
    for i in range(len(relisa)):
        ...
        finalcoords1 = ((
            (uv_temp1 * 0.5 + 0.5) - torch.tensor([[minx, miny]]).to(device)) /
                        edge_size) * 1000

        finalcoords2 = ((
            (uv_temp2 * 0.5 - 0.5) - torch.tensor([[minx2, miny2]]).to(device))
                        / edge_size2) * 1000
        ...
        colors1.append(edit_frame[relisa[i], reljsa[i], :] *
                       alpha.detach().cpu().numpy())

        colors2.append(edit_frame[relisa[i], reljsa[i], :] *
                       (1 - alpha).detach().cpu().numpy())
...
def main(training_folder, frame_edit, frames_folder, mask_rcnn_folder,
         frame_edit_file, edit_tex1_file, edit_tex2_file, frame_edit_index,
         output_folder, video_name, edit_frame_foreground,
         edit_frame_background, runinng_command):
    ...
    model_F_atlas = IMLP(input_dim=2,
                         output_dim=3,
    ...
    if frame_edit:
        ...
        edit_im1, edit_im2 = texture_edit_from_frame_edit(
            edit_frame, frame_number, model_F_mapping1, model_F_mapping2,
            model_alpha, number_of_frames, mask_frames, edit_frame_foreground,
            edit_frame_background, device)
        ...
        edited_frame = edit_frame[:, :, :3] * (
            edit_frame[:, :, 3][:, :, np.newaxis]) + edited_frame[:, :, :3] * (
    ...
    output_folder_final = os.path.join(output_folder,
                                       '%s_%06d' % (video_name, folder_time))
    ...
    apply_edit(model_F_atlas,
               ...
               alpha_im2=alpha_im2)
    if frame_edit:
        imageio.imwrite("%s/the_edited_frame.png" % output_folder_final,
                        (edited_frame * 255).astype(np.uint8))
        imageio.imwrite("%s/the_input_edit_frame.png" % output_folder_final,
                        (edit_frame * 255).astype(np.uint8))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process some integers.')

    parser.add_argument('--trained_model_folder',
                        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        'the folder that contains the masks produced by Mask-RCNN and the images '
    )
    ...
    parser.add_argument('--use_edit_frame',
                        type=bool,
                        nargs='?',
                        const=True,
                        help='if true, the code expects an edit of one frame',
                        default=False)

    parser.add_argument(
        '--edit_frame_foreground',
        type=bool,
        nargs='?',
        const=True,
        help='if true, the edit is applied only on the foreground',
        default=False)

    parser.add_argument(
        '--edit_frame_background',
        type=bool,
        nargs='?',
        const=True,
        help='if true, the edit is applied only on the background',
        default=False)
    ...
    parser.add_argument(
        ...
        type=int,
        help=
        'if use_edit_frame==true, the code needs the frame index that should be edited'
    )
    ...
    parser.add_argument('--edit_foreground_path',
                        type=str,
                        help='the path to the foreground texture edit')
    parser.add_argument('--edit_background_path',
                        type=str,
    ...
    main(training_folder, frame_edit, frames_folder, mask_rcnn_folder,
         frame_edit_file, edit_tex1_file, edit_tex2_file, frame_edit_index,
         output_folder, video_name, edit_frame_foreground,
         edit_frame_background, ' '.join(sys.argv))
