...
def get_colors(resolution, minx, maxx, miny, maxy, pointx, pointy, image):
    ...
    pos_logicaly = np.logical_and(
        np.ceil(pointy2) >= 0, np.floor(pointy2) >= 0)
    pos_logicalx = np.logical_and(
        np.ceil(pointx2) >= 0, np.floor(pointx2) >= 0)
...
def get_mapping_area(model_F_mapping, F_alpha, mask_frames, resx, number_of_frames, uv_shift, device,
    ...
    relisa = np.array_split(
        relis_i.numpy(), np.ceil(relis_i.shape[0] / 100000))
    reljsa = np.array_split(
        relis_j.numpy(), np.ceil(relis_i.shape[0] / 100000))
    relfsa = np.array_split(
        relis_f.numpy(), np.ceil(relis_i.shape[0] / 100000))
    ...
    with torch.no_grad():
        for i in range(len(relisa)):
            ...
            relfs = torch.from_numpy(relfsa[i]).unsqueeze(
                1) / (number_of_frames / 2) - 1
...
def evaluate_model(model_F_atlas, resx, resy, number_of_frames, model_F_mapping1, model_F_mapping2, model_alpha,
    ...
    checkerboard_ = np.array(Image.open(
        str("checkerboard.png"))).astype(np.float64) / 255.
    ...
    video_frames_reconstruction_edit1 = np.zeros(
        (resy, resx, 3, number_of_frames))
    video_frames_reconstruction_edit1_checkerboard = np.ones(
        (resy, resx, 3, number_of_frames)) * 0.5
    video_frames_reconstruction_edit2 = np.zeros(
        (resy, resx, 3, number_of_frames))
    video_frames_reconstruction_edit2_checkerboard = np.ones(
        (resy, resx, 3, number_of_frames)) * 0.5
    ...
    with torch.no_grad():
        for f in range(number_of_frames):
            ...
            relisa = np.array_split(
                relis_i.numpy(), np.ceil(relis_i.shape[0] / 100000))
            reljsa = np.array_split(
                reljs_i.numpy(), np.ceil(relis_i.shape[0] / 100000))
            ...
            for i in range(len(relisa)):
                relis = torch.from_numpy(relisa[i]).unsqueeze(
                    1) / (larger_dim / 2) - 1
                reljs = torch.from_numpy(reljsa[i]).unsqueeze(
                    1) / (larger_dim / 2) - 1
                ...
                rigidity_loss1 = get_rigidity_loss(
                    ...
                    model_F_mapping1,
                    uv_temp1, device,
                    uvw_mapping_scale=uv_mapping_scale, return_all=True)
                ...
                uv1_frames_reconstruction[relisa[i],
                                          reljsa[i], 0, f] = uv_temp1[:, 0]
                uv1_frames_reconstruction[relisa[i],
                                          reljsa[i], 1, f] = uv_temp1[:, 1]

                uv2_frames_reconstruction[relisa[i],
                                          reljsa[i], 0, f] = uv_temp2[:, 0]
                uv2_frames_reconstruction[relisa[i],
                                          reljsa[i], 1, f] = uv_temp2[:, 1]
                ...
                m2_alpha_alpha.append(
                    1.0 - alpha.cpu().squeeze()[relevant2].numpy())
                ...
                try:
                    ...
                    masks2[np.floor(pointsy2).astype((np.int64)), np.floor(
                        pointsx2).astype((np.int64))] = 1
                    masks2[np.floor(pointsy2).astype((np.int64)), np.ceil(
                        pointsx2).astype((np.int64))] = 1
                    masks2[np.ceil(pointsy2).astype((np.int64)),
                ...
                video_frames_reconstruction_edit1[relisa[i][relevant1], reljsa[i][relevant1], :, f] = rgb21 * \
                    alpha.cpu().numpy()[
                    relevant1]
                video_frames_reconstruction_edit2[relisa[i]
                                                  [relevant2], reljsa[i][relevant2], :, f] = rgb22
                ...
                flow_alpha_loss_video[relisa[i], reljsa[i],
            ...
            if show_atlas_alpha:  # this part is slow, not running during training
                cur_mask1 = interpolate_alpha(
                    m1_alpha_v, m1_alpha_u, m1_alpha_alpha)
                cur_mask2 = interpolate_alpha(
                    m2_alpha_v, m2_alpha_u, m2_alpha_alpha)
    ...
    for i in range(number_of_frames):
        ...
        cc = np.concatenate(
            (cur, cv2.resize(masks1[:, :, np.newaxis] *
             edited_tex1.numpy(), (cur.shape[0], cur.shape[0]))),
            axis=1)
        ...
        cc = np.concatenate(
            (cur, cv2.resize(checkerboard2.numpy(),
             (cur.shape[0], cur.shape[0]))),
            axis=1)
        ...
        cc = np.concatenate(
            (cur, cv2.resize(masks2[:, :, np.newaxis] *
             edited_tex2.numpy(), (cur.shape[0], cur.shape[0]))),
            axis=1)
    ...
    file1 = open('%s/%06d/PSNR_%f' %
                 (results_folder, iteration, pnsrs.mean()), "a")
    file1.close()
    if save_checkpoint:
        writer.add_image("Train/atlas1", masks1[:, :, np.newaxis]
                         * texture_orig1.numpy(), iteration, dataformats='HWC')
        writer.add_image("Train/atlas2", masks2[:, :, np.newaxis]
                         * texture_orig2.numpy(), iteration, dataformats='HWC')
        writer.add_image(
            "Train/recon_frame_0",
