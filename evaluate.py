...
def interpolate_alpha(m1_alpha_v, m1_alpha_u, m1_alpha_alpha):
    ...
    masks1 = griddata((m1_alpha_u, m1_alpha_v),
                      m1_alpha_alpha, (xv, yv),
                      method='linear')
    return masks1
...
def get_colors(resolution, minx, maxx, miny, maxy, mind, maxd, pointx, pointy,
               pointd, image):
    pixel_size = resolution / (maxx - minx)
    # Change uvw to pixel coordinates of the discretized image
    pointx2 = ((pointx - minx) * pixel_size).numpy()
    pointy2 = ((pointy - miny) * pixel_size).numpy()
    pointd2 = ((pointd - mind) * pixel_size).numpy()
    ...
    mx_logicaly = np.logical_and(
        np.ceil(pointy2) < resolution,
        np.floor(pointy2) < resolution)
    mx_logicaxlx = np.logical_and(
        np.ceil(pointx2) < resolution,
        np.floor(pointx2) < resolution)
...
def get_high_res_texture(resolution, num_slices, minx, maxx, miny, maxy, mind,
    ...
    with torch.no_grad():
        ...
        #                 (10, ii), cv2.FONT_HERSHEY_SIMPLEX, 1.2, cur_color, 2,
        #                 cv2.LINE_AA)
        #     cv2.putText(reconstruction_texture2.numpy(),
        #                 'ABCDEFGHIJKLMNOPQRSTUVWXYZ?~;:<./\|][{},',
        #                 (10, ii + 40), cv2.FONT_HERSHEY_SIMPLEX, 1.1,
        ...
        #     cv2.putText(reconstruction_texture2.numpy(),
        #                 'abcdefghijlmnopqrstuvwxyz1234567890!@#$%^&*()-+=>',
        #                 (10, ii + 500), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
        #                 cur_color, 2, cv2.LINE_AA)
        #     cv2.putText(reconstruction_texture2.numpy(),
        #                 'ABCDEFGHIJKLMNOPQRSTUVWXYZ?~;:<./\|][{},',
...
def get_img_from_fig(fig, dpi=180):
    """
    translating the fig into np array according to a certain dpi
    """
    buf = io.BytesIO()
    ...
    with torch.no_grad():
        for i in range(len(relisa)):
            ...
            if torch.any(alpha > alpha_thresh):
                uvw = uvw * 0.5 + uvw_shift
                curminx = torch.min(uvw[alpha > alpha_thresh, 0])
                curminy = torch.min(uvw[alpha > alpha_thresh, 1])
                curmind = torch.min(uvw[alpha > alpha_thresh, 2])
                curmaxx = torch.max(uvw[alpha > alpha_thresh, 0])
                curmaxy = torch.max(uvw[alpha > alpha_thresh, 1])
                curmaxd = torch.max(uvw[alpha > alpha_thresh, 2])
                minx = torch.min(torch.tensor([curminx, minx]))
                miny = torch.min(torch.tensor([curminy, miny]))
                mind = torch.min(torch.tensor([curmind, mind]))
                maxx = torch.max(torch.tensor([curmaxx, maxx]))
                maxy = torch.max(torch.tensor([curmaxy, maxy]))
                maxd = torch.max(torch.tensor([curmaxd, maxd]))

    maxx = np.minimum(maxx, 1)
    maxy = np.minimum(maxy, 1)
    maxd = np.minimum(maxd, 1)
...
# for visualizing uv images their values are mapped to the range (0,1) by using (edge_size, minx, miny,mind)
...
def normalize_uvw_images(uvw_frames_reconstruction, values_shift, edge_size,
    ...
    uvw_frames_reconstruction[:, :, 1, :] = (
        (uvw_frames_reconstruction[:, :, 1, :] * 0.5 + values_shift) -
        np.float64(miny)) / edge_size
    uvw_frames_reconstruction[:, :, 2, :] = (
        (uvw_frames_reconstruction[:, :, 2, :] * 0.5 + values_shift) -
        np.float64(mind)) / edge_size
    uvw_frames_reconstruction[uvw_frames_reconstruction > 1] = 1
    uvw_frames_reconstruction[uvw_frames_reconstruction < 0] = 0
    return uvw_frames_reconstruction
...
def evaluate_model(model_F_atlas,
    ...
    # alpha_reconstruction = np.zeros((resy, resx, resd, number_of_frames))
