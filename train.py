...
def main(config):
    ...
    # the folder (under the code's folder where the experiments will be saved.
    results_folder_name = config["results_folder_name"]
    # for each experiment folder (saved inside "results_folder_name") add this string
    ...
    if not load_checkpoint:
        ...
        model_F_mapping2.load_state_dict(
            init_file["model_F_mapping2_state_dict"])
    ...
    for i in range(start_iteration, iters_num):
        ...
        alpha = 0.5 * (model_alpha(xydt_current) + 1.0)
