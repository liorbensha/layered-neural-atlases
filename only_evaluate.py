...
def main(training_folder, frames_folder, mask_rcnn_folder, output_folder,
         video_name, runinng_command):
    # read config:
    ...
    optical_flows_mask, video_frames, _, mask_frames, _, _, _, optical_flows, depth_frames = load_input_data(
    ...
    Path(os.path.join(output_folder, '%s_%06d' %
                      (video_name, folder_time))).mkdir(parents=True,
                                                        exist_ok=True)
    file1 = open(
        "%s/runinng_command" %
